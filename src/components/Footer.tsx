      ...
      <div className="max-w-7xl mx-auto flex flex-col items-center space-y-14 md:space-y-20">
        ...
        <Reveal className="w-full max-w-2xl">
          <nav
            ...
            className="w-full rounded-2xl border border-brand-primary/[0.08] bg-white/50 px-4 py-6 shadow-[0_1px_0_rgba(29,29,27,0.04)] backdrop-blur-[2px] md:px-6 md:py-7"
          >
            <p className="mb-5 text-center font-serif text-[11px] italic text-brand-muted/90 md:text-xs">
              Explora el sitio
            </p>
            <Stagger fast className="grid grid-cols-2 gap-2 sm:grid-cols-4 sm:gap-3">
              {FOOTER_NAV.map(({ href, label, hint, Icon }) => (
                <StaggerItem key={href} className="min-w-0">
                  ...
                  >
                    <span className="flex h-11 w-11 items-center justify-center rounded-full border border-brand-primary/[0.09] bg-brand-secondary/80 text-brand-primary/45 transition-all duration-300 group-hover:border-brand-accent/40 group-hover:bg-white group-hover:text-brand-accent group-hover:-translate-y-0.5 md:h-12 md:w-12">
        ...
        <Reveal className="text-center space-y-6" delay={0.08}>
          <h4
            className="text-4xl md:text-6xl tracking-[0.2em] text-brand-primary uppercase transition-colors duration-500 ease-[cubic-bezier(0.16,1,0.3,1)] hover:text-brand-accent cursor-default"
        ...
        <Reveal className="flex flex-col items-center gap-10" delay={0.12}>
          <SiteSocialIconRow />

          <button
            type="button"
            onClick={scrollToTop}
            className="group flex flex-col items-center gap-2 editorial-spacing opacity-30 hover:opacity-100 transition-all duration-400 hover:-translate-y-1"
          >
            <ArrowUp size={16} strokeWidth={1.5} className="transition-transform duration-400 group-hover:-translate-y-0.5" />
            HACIA ARRIBA
          </button>
        </Reveal>

        <Reveal className="w-full pt-16 border-t border-brand-primary/5 flex flex-col md:flex-row justify-between items-center gap-8" delay={0.16}>
          ...
          <div className="flex gap-12 text-[10px] font-sans font-bold tracking-widest text-brand-muted uppercase">
            <Link href="/privacidad" className="nav-link-underline hover:text-brand-primary">PRIVACIDAD</Link>
            <Link href="/terminos" className="nav-link-underline hover:text-brand-primary">TÉRMINOS</Link>
          </div>
        </Reveal>
      </div>
    </footer>
